# This code is adapted from Karras et al. (EDM, https://github.com/NVlabs/edm/blob/main/dataset_tool.py).
# The original code is licensed under a Creative Commons
# Attribution-NonCommercial-ShareAlike 4.0 International License.
#
# We add functions for MNIST datasets and CIFAR-10 test dataset,
# and functions for creating label noise datasets.
...
def open_cifar10_test(tarball: str, *, max_images: Optional[int]):
    images = []
    labels = []
...
def open_cifar100(tarball: str, *, max_images: Optional[int]):
    ...
    with tarfile.open(tarball, 'r:gz') as tar:
        ...
        coarse_labels.append(data['coarse_labels'])
    ...
    def iterate_images():
        for idx, img in enumerate(images):
            yield dict(img=img, label=int(labels[idx]), coarse_labels=int(coarse_labels[idx]))
            if idx >= max_idx - 1:
                break
...
def open_cifar100_test(tarball: str, *, max_images: Optional[int]):
    images = []
    labels = []
    coarse_labels = []

    with tarfile.open(tarball, 'r:gz') as tar:
        member = tar.getmember(f'cifar-100-python/test')
        with tar.extractfile(member) as file:
            data = pickle.load(file, encoding='latin1')
        images.append(data['data'].reshape(-1, 3, 32, 32))
        labels.append(data['fine_labels'])
...
def open_mnist_test(images_gz: str, *, max_images: Optional[int]):
    labels_gz = images_gz.replace('-images-idx3-ubyte.gz', '-labels-idx1-ubyte.gz')
    assert labels_gz != images_gz
    images = []
    labels = []
...
def open_dataset(source, *, max_images: Optional[int], eval=False):
    if os.path.isdir(source):
        ...
        if os.path.basename(source) == 'cifar-10-python.tar.gz':
            if eval:
                return open_cifar10_test(source, max_images=max_images)
            else:
                return open_cifar10(source, max_images=max_images)
        elif os.path.basename(source) == 'cifar-100-python.tar.gz':
            if eval:
                return open_cifar100_test(source, max_images=max_images)
            else:
            ...
            return open_mnist_test(source, max_images=max_images)
...
@click.command()
# @click.option('--source',     help='Input directory or archive name', metavar='PATH',   type=str, default='/data/xche264/datasets/cifar100/cifar-100-python.tar.gz')
# @click.option('--dest',       help='Output directory or archive name', metavar='PATH',  type=str, default='/data/xche264/datasets/cifar100/cifar100_sym_20-32x32.zip')
...
@click.option('--noise_type',  help='Noise type', metavar='MODE',                        type=click.Choice(['sym', 'asym']),default='sym')
@click.option('--noise_rate',  help='Noise rate', metavar='FLOAT',                       type=float, default=0.0)
@click.option('--eval',        help='Load evaluation dataset', metavar='BOOL',           type=bool, default=False)
...
def main(
    ...
    num_files, input_iter = open_dataset(source, max_images=max_images, eval=eval)
    ...
    if 'mnist' in dest:
        ...
        # assym: 2->7, 3->8, 5->6, 6->5
        noise_label_list = [0, 1, 7, 8, 4, 6, 5, 7, 8, 9]
        change_label_list = [2, 3, 5, 6]
    elif 'cifar100' in dest:
        num_classes = 100

        # assym: circularly within superclass
        c_label = np.array([4, 1, 14, 8, 0, 6, 7, 7, 18, 3,
        ...
        coarse_label_data = np.zeros((20, 5), dtype=int)
        for i in range(20):
            coarse_label_data[i] = np.where(c_label == i)[0]

        noise_label_list = np.zeros(100, dtype=int)
        for i in range(100):
            target = coarse_label_data[c_label[i]].tolist()
            noise_label_list[i] = int(target[(target.index(i) + 1) % 5])
        noise_label_list = noise_label_list.tolist()
        change_label_list = np.arange(100).tolist()
    elif 'cifar10' in dest:
        num_classes = 10

        # assym: 9->1, 2->0, 4->7, 3<->5 (truck -> automobile, bird -> airplane, deer -> horse, cat <-> dog)
        noise_label_list = [0, 1, 0, 5, 7, 3, 6, 7, 8, 1]
        change_label_list = [9, 2, 4, 3, 5]
    else:
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        if image['label'] is None:
            ...
            if noise_type == 'sym':
                if np.random.rand(1) < noise_rate:
                    candidate = np.delete(np.arange(num_classes), image['label'])
                    label = np.random.choice(candidate).item()
                else:
                    label = image['label']
            elif noise_type == 'asym':
                if image['label'] in change_label_list:
                    if np.random.rand(1) < noise_rate:
                        label = noise_label_list[image['label']]
                    else:
                        label = image['label']
                else:
                    label = image['label']
            else:
                raise NotImplementedError(f'wrong noise type {noise_type}.')
