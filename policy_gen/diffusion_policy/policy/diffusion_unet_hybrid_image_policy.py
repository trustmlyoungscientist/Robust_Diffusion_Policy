...
class RobustDiffusionUnetHybridImagePolicy(DiffusionUnetHybridImagePolicy):
    def __init__(self, *args, **kwargs):
        # 直接调用父类的初始化方法
        super().__init__(*args, **kwargs)
    
    def compute_loss(self, batch, pseudo_obs, epoch=0, beta = 0.2, training=False, stop_epoch=100, scale=0.1):
        # normalize input
        assert 'valid_mask' not in batch
        nobs = self.normalizer.normalize(batch['obs'])
        nactions = self.normalizer['action'].normalize(batch['action'])
        ...
        if training and epoch<=stop_epoch:
            t0_cond = torch.randn(pseudo_obs[index].shape, device=trajectory.device) * 0.5
        ...
        if not pseudo_obs == None and training and epoch<=stop_epoch:
            pseudo_obs[index] = (1 - beta) * pseudo_obs[index] + beta * pred_cond.data.detach()
        ...
        if training and epoch<=stop_epoch:
            ...
            loss_cond =  F.mse_loss(noisy_cond, pseudo_obs[index].detach(), reduction='none')
            loss = loss.mean() + loss_cond.mean()
        return loss
